...
class QHEXEDIT_API QHexEdit : public QAbstractScrollArea
{
    ...
    */
    ...
    set this property true to avoid horizontal scrollbars and show the maximal possible data. Default value is false*/
...
public:
    ...
    */
    ...
    /*! Replaces \param ba.size() bytes with a byte array \param ba
    \param pos Index position, where to overwrite
    \param ba QByteArray, which is inserted
    The data is overwritten and the data size is not changed.
    */
...
private:
    ...
    int _pxCharWidth, _pxCharHeight;            // char dimensions (dependent on font)
